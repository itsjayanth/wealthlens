 ...
 * randomized fake data for Phase 1, before a real Sharekhan SDK integration
 * lands. "Deterministic" here means: given the same ticker, the seeded PRNG
 * always produces the same base price / sector / historical curve, so the
 * mock behaves consistently across requests within a run rather than being
 * pure noise.
 */
...
export class MockShareKhanAdapter implements ShareKhanAdapter {
  async fetchHoldings(
    _credentials: BrokerCredentials
  ): Promise<BrokerHolding[]> {
    // Phase 1 mock: every linked client is simulated as holding a subset of
    // the fixed ticker universe. Quantities/avg buy prices are seeded off
    // the client code so a given client sees stable holdings across calls.
    const seedBase = hashString(_credentials.clientCode || "default");
  ...
  async fetchQuote(ticker: string): Promise<BrokerQuote> {
