...
import type { BrokerHistoricalPoint } from "./marketData";

/**
 * Rule-based v1 recommendation engine, ported from the real backend's engine
 * (apps/api/src/engine/recommendationEngine.ts) to run client-side against
 * the mock market data in ./marketData.
 *
 * Rules:
 ...
 * data via the provided `fetchHistorical` function. Returns recommendations
 * without id/status/timestamp - the caller fills those in when persisting.
 */
export function generateRecommendations(
  holdings: EngineHoldingInput[],
  fetchHistorical: (ticker: string, days: number) => BrokerHistoricalPoint[]
  ...
  for (const holding of holdings) {
    const historical = fetchHistorical(
      holding.ticker,
